...
class TwlTool(Observer, tk.Tk):
    ...
    def update_observer(self, component_id: str = "", attribute_id: str = ""):
        model = TwlApp.model()
        tab_state = tk.NORMAL if model.is_stat_det() and model.is_stable() and not model.is_empty() and model.has_three_reaction_forces() and len(model.forces) > 0 else tk.DISABLED
